...
class _OverlayMaskPosition extends State<OverlayMaskPosition> {
  ...
  void initState() {
    ...
    if (widget.initialMasks != null && widget.initialMasks!.isNotEmpty) {
      _maskConfigs = List<MaskConfig>.from(widget.initialMasks!);
    } else {
      _maskConfigs = [
        MaskConfig(
  ...
  @override
  void didUpdateWidget(OverlayMaskPosition oldWidget) {
    super.didUpdateWidget(oldWidget);
    
    // Atualiza as máscaras se forem passadas novas do componente pai
    if (widget.initialMasks != null && 
        widget.initialMasks!.isNotEmpty &&
        widget.initialMasks != oldWidget.initialMasks) {
      setState(() {
        _maskConfigs = List<MaskConfig>.from(widget.initialMasks!);
      });
    }
    
    // Atualiza a máscara selecionada se for mudada pelo componente pai
    if (widget.initialSelectedMaskId != oldWidget.initialSelectedMaskId &&
